...
__ALIGN_BEGIN static uint8_t CUSTOM_HID_ReportDesc_FS[USBD_CUSTOM_HID_REPORT_DESC_SIZE] __ALIGN_END = {
    ...
    0x75,
    0x08,  //   REPORT_SIZE (8)        传输字段的宽度为8bit，表示每个传输的数据范围�??0~ffff ffff
...
USBD_CUSTOM_HID_ItfTypeDef USBD_CustomHID_fops_FS = {CUSTOM_HID_ReportDesc_FS, CUSTOM_HID_Init_FS, CUSTOM_HID_DeInit_FS,
...
void SaveMixValueToFlash(void) {
  ...
  switch (USB_Recive_Buffer[0]) {
    ...
    case UID_BYTES_ID: {
      if (USB_Recive_Buffer[1] == 0 && USB_Recive_Buffer[2] == 0 && USB_Recive_Buffer[3] == 0 &&
          USB_Recive_Buffer[4] == 0 && USB_Recive_Buffer[5] == 0 && USB_Recive_Buffer[6] == 0) {
