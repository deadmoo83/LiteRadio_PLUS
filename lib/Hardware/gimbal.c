...
uint16_t Get_GimbalValue(gimbalChannelTypeDef channel) {
  ...
  if ((channel == RUDDER) || (channel == ELEVATOR) || (channel == AILERON) || (channel == THROTTLE)) {
    ...
    if (ADTemp >= Sampling_MaxMinData[channel][MIDDAT]) {
      outputTemp = (uint16_t) (CHANNEL_OUTPUT_MID +
                               (((float) ADTemp - Sampling_MaxMinData[channel][MIDDAT]) *
                                ((((float) CHANNEL_OUTPUT_MAX - CHANNEL_OUTPUT_MID)) /
                                 (Sampling_MaxMinData[channel][MAXDAT] - Sampling_MaxMinData[channel][MIDDAT]))));
    } else {
      outputTemp = (uint16_t) (CHANNEL_OUTPUT_MID -
                               (((float) Sampling_MaxMinData[channel][MIDDAT] - ADTemp) *
                                ((((float) CHANNEL_OUTPUT_MID - CHANNEL_OUTPUT_MIN)) /
                                 (Sampling_MaxMinData[channel][MIDDAT] - Sampling_MaxMinData[channel][MINDAT]))));
    }
...
void gimbalTask(void *param) {
  ...
  while (1) {
    ...
    gimbalEvent = xEventGroupWaitBits(gimbalEventHandle, GIMBAL_CALIBRATE_IN, pdTRUE, pdFALSE, 0);
