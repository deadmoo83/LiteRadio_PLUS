...
void Buzzer_BeeNumInit(uint8_t buzzerCountInit) { buzzerCountCurr = buzzerCountInit; }
...
void Buzzer_Start() { HAL_TIM_PWM_Start(&htim3, BUZZER_PWM_CH); }

void Buzzer_Stop() { HAL_TIM_PWM_Stop(&htim3, BUZZER_PWM_CH); }

void HAL_TIM_SET_COMPARE(uint16_t compare) { htim3.Instance->CCR1 = compare; }
...
void buzzerTask(void *param) {
  ...
  while (1) {
    ...
    buzzerEvent = xEventGroupWaitBits(buzzerEventHandle,
                                      POWER_ON_RING | POWER_OFF_RING | SETUP_MID_RING | SETUP_MINMAX_RING |
                                          SETUP_END_RING | LOW_ELECTRICITY_RING | RISS_WARNING_RING,
                                      pdTRUE, pdFALSE, 0);
