...
void ProcessTLMpacket() {
  ...
  switch (TLMheader & ELRS_TELEMETRY_TYPE_MASK) {
    case ELRS_TELEMETRY_TYPE_LINK:
      ...
      // linkStatistics.downlink_Link_quality = LPD_DownlinkLQ.update(LQCalc.getLQ()) + 1; // +1 fixes rounding issues
      // with filter and makes it consistent with RX LQ Calculation
...
void GenerateSyncPacketData() {
  ...
  expresslrs_tlm_ratio_e newRatio =
      (StubbornSender_IsActive()) ? TLM_RATIO_1_2 : (expresslrs_tlm_ratio_e) tx_config.tlm;
...
void SetRFLinkRate(uint8_t index)  // Set speed of RF link (hz)
{
  ...
  if ((ModParams == ExpressLRS_currAirRate_Modparams) && (RFperf == ExpressLRS_currAirRate_RFperfParams) &&
...
uint16_t SendRCdataToRF(uint16_t *crsfcontrol_data) {
  ...
  SyncInterval = (connectionState == connected) ? ExpressLRS_currAirRate_RFperfParams->SyncPktIntervalConnected
  ...
  //    uint8_t NonceFHSSresultWindow = (NonceFHSSresult == 1 || NonceFHSSresult == 2) ? 1 : 0; // restrict to the
  //    middle nonce ticks (not before or after freq chance)
  ...
  if ((syncSpamCounter || WithinSyncSpamResidualWindow) && (NonceFHSSresult == 1 || NonceFHSSresult == 2)) {
    ...
    syncSlot = 0;
  } else if ((!skipSync) && ((syncSlot / 2) <= NonceFHSSresult) && (now - SyncPacketLastSent > SyncInterval) &&
...
void ParamUpdateReq() { UpdateParamReq = 1; }
...
void ExpressLRS_Init(uint8_t protocolIndex) { HAL_Delay(1); }
