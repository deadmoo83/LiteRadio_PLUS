...
uint32_t Get_ProtocolDelayTime() { return protocolDelayTime; }
...
void statusTask(void *param) {
  ...
  while (1) {
    ...
    batteryWarningStatus =
        Cal_ElectricRelay(electricityADCvalue, batteryWarningStatus, downElectricityLimit, upElectricityLimit);
    ...
    keyEvent = xEventGroupWaitBits(KeyEventHandle, POWERSWITCH_LONG_PRESS | BIND_SHORT_PRESS | SETUP_SHORT_PRESS,
                                   pdTRUE, pdFALSE, 0);
    ...
    gimbalEvent = xEventGroupWaitBits(gimbalEventHandle, GIMBAL_CALIBRATE_END, pdTRUE, pdTRUE, 0);
    ...
    switch (RCstatus) {
      ...
      case RC_CHRG_AND_JOYSTICK: {
        ...
        } else if (internalCRSFdata.lastConfigStatus == CONFIG_CRSF_ON &&
                   internalCRSFdata.configStatus == CONFIG_CRSF_OFF) {
          vTaskSuspend(radiolinkTaskHandle);
          HAL_TIM_Base_Stop_IT(&htim1);
        ...
        if (externalCRSFdata.lastConfigStatus == CONFIG_CRSF_OFF && externalCRSFdata.configStatus == CONFIG_CRSF_ON) {
          ...
          HAL_GPIO_WritePin(EXTERNAL_RF_EN_GPIO_Port, EXTERNAL_RF_EN_Pin, GPIO_PIN_SET);
        } else if (externalCRSFdata.lastConfigStatus == CONFIG_CRSF_ON &&
