...
/*继电器特性*/
uint8_t Cal_ElectricRelay(uint16_t currentValue, uint8_t currentStatus, uint16_t downValueLimit,
                          uint16_t upValueLimit) {
  if (currentStatus) {
...
uint16_t GetUniqueID(void) {
  ...
  ID = (ID << 8) + chipID.IDbyte[1] + chipID.IDbyte[3] + chipID.IDbyte[5] + chipID.IDbyte[7] + chipID.IDbyte[9] +
       chipID.IDbyte[11];
  return ID;
}
...
void GetSbusPackage(uint8_t *channelToSBUS, uint16_t *controlDataBuff) {
  ...
  channelToSBUS[2] =
      (uint8_t) ((channelDataBuff[MIX_AILERON] & 0x07FF) >> 8 | (channelDataBuff[MIX_ELEVATOR] & 0x07FF) << 3);
  channelToSBUS[3] =
      (uint8_t) ((channelDataBuff[MIX_ELEVATOR] & 0x07FF) >> 5 | (channelDataBuff[MIX_THROTTLE] & 0x07FF) << 6);
  channelToSBUS[4] = (uint8_t) ((channelDataBuff[MIX_THROTTLE] & 0x07FF) >> 2);
  channelToSBUS[5] =
      (uint8_t) ((channelDataBuff[MIX_THROTTLE] & 0x07FF) >> 10 | (channelDataBuff[MIX_RUDDER] & 0x07FF) << 1);
