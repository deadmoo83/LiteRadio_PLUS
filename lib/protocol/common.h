...
#define FIRMWARE_PATCH_VERSION (0x01U)
typedef struct {
...
typedef enum { SENDER_IDLE = 0, SENDING, SEND_NEXT, WAIT_UNTIL_NEXT_CONFIRM, RESYNC } stubborn_sender_state_s;
...
typedef enum { bad_sync_retry = 4, bad_sync = 3, connected = 2, tentative = 1, disconnected = 0 } connectionState_e;

typedef enum { tim_disconnected = 0, tim_tentative = 1, tim_locked = 2 } RXtimerState_e;

extern connectionState_e connectionState;
extern connectionState_e connectionStatePrev;

typedef enum { RF_DOWNLINK_INFO = 0, RF_UPLINK_INFO = 1, RF_AIRMODE_PARAMETERS = 2 } expresslrs_tlm_header_e;
...
typedef struct expresslrs_rf_pref_params_s {
  ...
  uint32_t
      SyncPktIntervalDisconnected;    // how often to send the SYNC_PACKET packet (ms) when there is no response from RX
...
typedef struct expresslrs_mod_settings_s {
  ...
  uint8_t FHSShopInterval;  // every X packets we hop to a new frequency. Max value of 16 since only 4 bits have been
                            // assigned in the sync package.
  uint8_t PreambleLen;
