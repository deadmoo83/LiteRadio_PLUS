...
static uint16_t channelDataBuff[16] = {1500, 1500, 1500, 1500, 1500, 1500, 1500, 1500,
...
void CRSF_SetBind() { HAL_Delay(1); }

void CRSF_Init(uint8_t protocolIndex) { HAL_Delay(1); }
...
uint16_t CRSF_Process(uint16_t *crsfcontrol_data) {
  ...
  if (externalCRSFdata.configSetFlag) {
    ...
    externalCRSFdata.configSetFlag = 0;
             ...
             ((HAL_GetTick() - LastCRSFProcessMillis) > intervalOfGetParameter)) {
    ...
    LastCRSFProcessMillis = HAL_GetTick();
    if (maxPackSize == 64)
      GetExternalRFParameter(power, 0);
    else if (maxPackSize == 32)
      GetExternalRFParameter(power, 1);
  } else if (externalRFprarmeter.rate == 0xff && ((HAL_GetTick() - LastCRSFProcessMillis) > intervalOfGetParameter)) {
    LastCRSFProcessMillis = HAL_GetTick();
    if (maxPackSize == 64)
      GetExternalRFParameter(rate, 1);
    else if (maxPackSize == 32)
      GetExternalRFParameter(rate, 2);
  } else if (externalRFprarmeter.TLM == 0xff && ((HAL_GetTick() - LastCRSFProcessMillis) > intervalOfGetParameter)) {
    LastCRSFProcessMillis = HAL_GetTick();
    if (maxPackSize == 64)
      GetExternalRFParameter(tlm, 0);
    else if (maxPackSize == 32)
      GetExternalRFParameter(tlm, 2);
  } else if (externalCRSFdata.regulatoryDomainIndex != 0 && externalRFprarmeter.power != 0xff &&
             externalRFprarmeter.rate != 0xff && externalRFprarmeter.TLM != 0xff) {
