...
uint8_t packet[40];
uint8_t phase;
const uint8_t CH_AETR[] = {AILERON, ELEVATOR, THROTTLE, RUDDER, CH5,  CH6,  CH7,  CH8,
                           CH9,     CH10,     CH11,     CH12,   CH13, CH14, CH15, CH16};
enum { SFHSS_START = 0x00, SFHSS_CAL = 0x01, SFHSS_DATA1 = 0x02, SFHSS_DATA2 = 0x03, SFHSS_TUNE = 0x04 };
...
static void __attribute__((unused)) SFHSS_build_data_packet(uint16_t *sfhssControlData) {
  ...
  // command.bit1 is unknown but seems to be linked to the payload[0].bit0 but more dumps are needed: payload[0]=0x82 ->
  // =0, payload[0]=0x81 -> =1 command.bit2 is the failsafe transmission indicator: =0 -> normal data, =1->failsafe data
  ...
  if (counter & 1)
    command |= 0x08;  // Transmit lower and upper channels twice in a row
...
static void __attribute__((unused)) SFHSS_send_packet() { CC2500_WriteData(packet, SFHSS_PACKET_LEN); }
...
void SFHSS_SetBind(void) { HAL_Delay(1); }
