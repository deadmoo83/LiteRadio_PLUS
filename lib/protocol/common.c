...
expresslrs_rf_pref_params_s ExpressLRS_AirRateRFperf[RATE_MAX] = {{0, RATE_200HZ, -112, 4380, 3000, 2500, 2000, 4000},
...
expresslrs_rf_pref_params_s ExpressLRS_AirRateRFperf[RATE_MAX] = {{0, RATE_500HZ, -105, 1665, 2500, 2500, 3, 5000},
...
void GenerateChannelDataHybridSwitch8(volatile uint8_t *Buffer, uint16_t *elrsControlData) {
  ...
  Buffer[5] =
      ((dataVal[0] & 0x06) << 5) | ((dataVal[1] & 0x06) << 3) | ((dataVal[2] & 0x06) << 1) | ((dataVal[3] & 0x06) >> 1);
...
void CRSF_UpdateSwitchValues(uint16_t *elrsControlData) {
  ...
  for (int i = 1; i < N_SWITCHES - 1; i++) {
    ...
    if (ch < (CRSF_CHANNEL_VALUE_MID - CHANNEL_BIN_SIZE / 4) || ch > (CRSF_CHANNEL_VALUE_MID + CHANNEL_BIN_SIZE / 4))
...
void CRSF_SetSentSwitch(uint8_t index, uint8_t value) { sentSwitches[index] = value; }
...
uint8_t CRSF_to_BIT(uint16_t val) { return (val > CRSF_CHANNEL_VALUE_MID) ? 1 : 0; }
...
uint8_t StubbornSender_IsActive() { return StubbornSender.senderState != SENDER_IDLE; }
...
uint32_t uidMacSeedGet(void) {
  const uint32_t macSeed = ((uint32_t) UID[2] << 24) + ((uint32_t) UID[3] << 16) + ((uint32_t) UID[4] << 8) + UID[5];
  return macSeed;
}
